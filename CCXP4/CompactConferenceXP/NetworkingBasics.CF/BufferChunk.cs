...
namespace CF.MSR.LST
{
    ...
    public class BufferChunk : IDisposable, ICloneable
    {
        ...
        private static void ValidateObject(object o)
        {
            if (o == null)
            {
                throw new ArgumentNullException("BufferChunk does not accept or create null objects");
            }
        }
        ...
        private static void ValidateNotZeroLength(int length)
        {
            if (length == 0)
            {
                throw new NoDataException("BufferChunk does not accept or create zero-length objects");
            }
        }
        ...
        private static void ValidateNonNegative(int val)
        {
            ...
            {
                /// $CompactConferenceXP
                /// No overload for method 'ArgumentOutOfRangeException' takes '3' arguments
        ...
        private static void ValidatePointerData(int index, int length, int dataLength)
        {
            ...
            {
                throw new ArgumentOutOfRangeException(string.Format("Index and Length do not point to valid data " +
                    "index:{0} + length:{1} must be <= dataLength:{2}", index, length, dataLength));
            }
        }
        ...
        private static void ValidateSufficientData(int requested, int actual)
        {
            if (requested > actual)
            {
                throw new InsufficientDataException(string.Format(
                    "BufferChunk does not have enough data to fulfill your request. " +
                    "Requested data:{0} Actual data:{1}", requested, actual));
            }
        }
        ...
        private static void ValidateSufficientSpace(int requested, int actual)
        {
            if (requested > actual)
            {
                throw new InsufficientSpaceException(string.Format(
                    "BufferChunk does not have enough buffer or data to fulfill your request. " +
                    "Requested space:{0} Actual space:{1}", requested, actual));
            }
        }
        ...
        private static void ValidateIntPtr(IntPtr ptr)
        {
            if (ptr == IntPtr.Zero)
            {
                throw new ArgumentException("Null pointers are not valid arguments.");
            }
        }
        ...
        //        private Int64 _GetInt64(int index)
        //        {
        //            Int64 ret;
        //            
        //            // BigEndian network -> LittleEndian architecture
        //            if(littleEndian)
        //            {
        //                ret  = ((Int64)buffer[this.index + index + 0]) << 7 * 8;
        //                ret += ((Int64)buffer[this.index + index + 1]) << 6 * 8;
        //                ret += ((Int64)buffer[this.index + index + 2]) << 5 * 8;
        //                ret += ((Int64)buffer[this.index + index + 3]) << 4 * 8;
        //                ret += ((Int64)buffer[this.index + index + 4]) << 3 * 8;
        //                ret += ((Int64)buffer[this.index + index + 5]) << 2 * 8;
        //                ret += ((Int64)buffer[this.index + index + 6]) << 1 * 8;
        //                ret += ((Int64)buffer[this.index + index + 7]) << 0 * 8;
        //            }
        //            else // BigEndian network -> BigEndian architecture
        //            {
        //                ret  = ((Int64)buffer[this.index + index + 0]) << 0 * 8;
        //                ret += ((Int64)buffer[this.index + index + 1]) << 1 * 8;
        //                ret += ((Int64)buffer[this.index + index + 2]) << 2 * 8;
        //                ret += ((Int64)buffer[this.index + index + 3]) << 3 * 8;
        //                ret += ((Int64)buffer[this.index + index + 4]) << 4 * 8;
        //                ret += ((Int64)buffer[this.index + index + 5]) << 5 * 8;
        //                ret += ((Int64)buffer[this.index + index + 6]) << 6 * 8;
        //                ret += ((Int64)buffer[this.index + index + 7]) << 7 * 8;
        //            }
        //            return ret;
        //        }
        //
        ...
        //        private void _SetInt64(int index, Int64 data)
        //        {
        //            // LittleEndian architecture -> BigEndian network
        //            if(littleEndian)
        //            {
        //                buffer[this.index + index + 0] = (byte)(data >> (7 * 8));
        //                buffer[this.index + index + 1] = (byte)(data >> (6 * 8));
        //                buffer[this.index + index + 2] = (byte)(data >> (5 * 8));
        //                buffer[this.index + index + 3] = (byte)(data >> (4 * 8));
        //                buffer[this.index + index + 4] = (byte)(data >> (3 * 8));
        //                buffer[this.index + index + 5] = (byte)(data >> (2 * 8));
        //                buffer[this.index + index + 6] = (byte)(data >> (1 * 8));
        //                buffer[this.index + index + 7] = (byte)(data >> (0 * 8));
        //            }
        //            else // BigEndian architecture -> BigEndian network
        //            {
        //                buffer[this.index + index + 0] = (byte)(data >> (0 * 8));
        ...
        //                buffer[this.index + index + 2] = (byte)(data >> (2 * 8));
        //                buffer[this.index + index + 3] = (byte)(data >> (3 * 8));
        //                buffer[this.index + index + 4] = (byte)(data >> (4 * 8));
        //                buffer[this.index + index + 5] = (byte)(data >> (5 * 8));
        //                buffer[this.index + index + 6] = (byte)(data >> (6 * 8));
        //                buffer[this.index + index + 7] = (byte)(data >> (7 * 8));
        //            }
        //        }
        //
