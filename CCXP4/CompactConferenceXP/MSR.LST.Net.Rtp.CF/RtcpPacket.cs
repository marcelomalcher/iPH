...
namespace CF.MSR.LST.Net.Rtp
{
    ...
    internal class DequeueEnumerator : IEnumerator
    {
        ...
        object IEnumerator.Current
        {
            get
            {
                if (index == -1)
                {
                    throw new InvalidOperationException("Call MoveNext() before calling Current()");
                }
    ...
    public class CompoundPacket : IEnumerable
    {
        ...
        private const int size = Rtp.MAX_PACKET_SIZE;
    ...
    internal class RtcpHeader : ICloneable, IRtcpData
    {
        ...
        public int ItemCount
        {
            ...
            set
            {
                ...
                if (value < 0 || value > 31)
                {
                    throw new RtcpHeaderException(string.Format("Invalid ItemCount:{0}, must be >=0 or <=31", value));
                }
        ...
        public short Length
        {
            ...
            set
            {
                ...
                {
                    throw new RtcpHeaderException(string.Format("Invalid length: {0}, must be >=0 or <= {1}",
    ...
    public class RtcpPacket : IRtcpData
    {
        ...
        protected static void ValidateItemSpace(int itemCount)
        {
            if (itemCount == 31)
            {
                throw new InsufficientItemSpaceException("Can not have more than 31 items in an RtcpPacket");
            }
        }
        ...
        protected static void ValidatePacketType(byte expected, byte actual)
        {
            if (actual != expected)
            {
                throw new RtcpPacketException(string.Format(
                    "Expected {0} pRtcpPacketExceptionacket: {1}, received: {2}",
                    (Rtcp.PacketType)expected, expected, actual));
            }
        }
        ...
        protected static void ValidateBufferBoundary(int cbBuffer)
        {
            ...
            {
                throw new RtcpPacketException(string.Format(
                    "The buffer does not end on a 32 bit boundary, buffer size: {0}, size % 4: {1}",
                    cbBuffer, cbBuffer % 4));
            }
        }
        ...
        {
            get { throw new RtcpPacketException("This method is meant to be overridden in a derived class"); }
        }
    ...
    /// NAME, EMAIL, etc.
    /// </summary>
    internal class SdesPacket : RtcpPacket, IRtcpData
    {
        #region Members
        ...
        /// </summary>
        public SdesPacket(uint ssrc, SdesData props)
            : base(Rtcp.PacketType.SDES)
        {
        ...
        /// </summary>
        public SdesPacket(RtcpPacket packet)
            : base(Rtcp.PacketType.SDES)
        {
        ...
        public override uint SSRC
        {
            set
            {
                throw new ApplicationException("An SDES packet may contain more than one SSRC." +
                    "Please use the  methods provided for appropriately manipulating SSRCs.");
            }
        }
        ...
        public override void WriteDataToBuffer(BufferChunk buffer)
        {
            ...
            foreach (SdesReport report in sdesReports)
            {
                WriteReportToBuffer(buffer, report.ssrc, report.sdes);
            }
    ...
    /// </summary>
    internal class ByePacket : RtcpPacket
    {
        #region Members
        ...
        /// </summary>
        public ByePacket(RtcpPacket packet)
            : base(Rtcp.PacketType.BYE)
        {
            ProcessPacket(packet);
        }
        ...
        public string Reason
        {
            ...
            set
            {
                ...
                else
                {
                    ...
                    {
                        throw new ApplicationException(string.Format("BYE reason must be <= {0} UTF8 bytes", 255));
                    }
                }
        ...
        public override uint SSRC
        {
            set
            {
                throw new ApplicationException("A BYE packet may contain more than one SSRC." +
                    "Please use the  methods provided for appropriately manipulating SSRCs.");
            }
        }
    ...
    /// </summary>
    internal class AppPacket : RtcpPacket
    {
        #region Static
        ...
        public AppPacket(uint ssrc, string name, byte subtype, byte[] data)
            : base(Rtcp.PacketType.APP)
        {
            SSRC = ssrc;
            Subtype = subtype;
            Name = name;
        ...
        public AppPacket(RtcpPacket packet)
            : base(Rtcp.PacketType.APP)
        {
            ProcessPacket(packet);
        }
            ...
            set
            {
                ...
                if (value == null || value == String.Empty)
                {
                    throw new ArgumentException(string.Format(
                        "Name is required, and must be {0} ASCII characters long", NAME_SIZE));
                }
                else
                {
                    ...
                    {
                        throw new ArgumentException(string.Format(
                            "Name must be {0} ASCII characters", NAME_SIZE));
                    }
                }
                ...
                if (value != null)
                {
                    ...
                    if (value.Length > Rtp.MAX_PACKET_SIZE - RtcpHeader.SIZE - Rtp.SSRC_SIZE - NAME_SIZE)
                    {
                        throw new ArgumentOutOfRangeException(string.Format(
                            "Data length must be <= Rtp.MAX_PACKET_SIZE({0}) - RtcpHeader.SIZE({1})" +
                            " - Rtp.SSRC_SIZE({2}) - NAME_SIZE({3})",
                            Rtp.MAX_PACKET_SIZE, RtcpHeader.SIZE, Rtp.SSRC_SIZE, NAME_SIZE));
                    }
    ...
    /// </summary>
    internal class RrPacket : RtcpPacket
    {
        #region Members
        ...
        /// </summary>
        /// <param name="packet">Packet to process</param>
        public RrPacket(RtcpPacket packet)
            : base(Rtcp.PacketType.RR)
        {
            ProcessPacket(packet);
        }
        ...
        public override string ToString()
        {
            string str = "ReceiverReportPacket {" +
                " SSRC := " + this.SSRC +
                " PacketType := " + PacketType +
                " ItemCount := " + this.ItemCount;

            foreach (ReceiverReport rr in this.ReceiverReports)
                str += " ReceiverReport := " + rr.ToString();
    ...
    /// </summary>
    internal class SrPacket : RtcpPacket
    {
        #region Members
        ...
        public SrPacket(uint ssrc)
            : base(Rtcp.PacketType.SR)
        {
            SSRC = ssrc;
        }

        public SrPacket(uint ssrc, SenderReport sr)
            : base(Rtcp.PacketType.SR)
        {
            if (sr == null)
            {
        ...
        /// </summary>
        /// <param name="packet">Packet to process</param>
        public SrPacket(RtcpPacket packet)
            : base(Rtcp.PacketType.SR)
        {
            sr = new SenderReport();
            ProcessPacket(packet);
        ...
        public override string ToString()
        {
            return "SenderReportPacket {" +
                ...
                " PacketType := " + PacketType +
                " ItemCount := " + this.ItemCount +
                " SenderReport := " + this.SenderReport.ToString() +
                " }";
        }
