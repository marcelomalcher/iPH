    ...
    public class SdesData : IRtcpData
    {
        ...
        public void SetTool(bool val)
        {
            ...
            if (val)            
            {
                ...
                /// $CompactConferenceXP:
                /// 'System.Reflection.Assembly' does not contain a definition for 'GetEntryAssembly'
                /// Replaced by GetCallingAssmbly
                /// > AssemblyName exe = Assembly.GetEntryAssembly().GetName();
                AssemblyName call = Assembly.GetCallingAssembly().GetName();
                AssemblyName rtp = Assembly.GetExecutingAssembly().GetName();

                tool = call.Name + " v" + call.Version + ", ";
            ...
            if (privs != null)
            {
                foreach (DictionaryEntry de in privs)
                {
                    ...
                    lock (utf8)
                    {
                        ...
                        prefix = utf8.GetString((byte[])de.Key, 0, ((byte[])de.Key).Length);
