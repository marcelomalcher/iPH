...
/// $CompactConferenceXP:
/// Added Reference to OpenNETCF.Diagnostics for EventLogEntryType enum
using OpenNETCF.Diagnostics;

namespace CF.MSR.LST.Net.Rtp
{
    /// <summary>
    ...
    public class RtpEvents
    {
        ...
        internal static void RaiseDuplicateCNameDetectedEvent(object[] args)
        {
            if (!FireEvent(DuplicateCNameDetected, args))
            {
                ...
                string message = string.Format("DuplicateCNameDetectedEvent - " +
                    " A duplicate of the local CName was detected on" +
                    " the network between {0} & {1}, RtpListener shutting down",
        ...
        internal static void RaiseFrameOutOfSequenceEvent(object[] args)
        {
            if (!FireEvent(FrameOutOfSequence, args))
            {
                ...
                eventLog.WriteEntry(string.Format("FrameOutOfSequenceEvent - RtpStream: {0}, " +
                    "LostFrames:{1}, Message:{2}", ea.RtpStream, ea.LostFrames, ea.Message),
                    EventLogEntryType.Warning, (int)RtpEL.ID.FrameOutOfSequence);
            }
        }
        ...
        internal static void RaiseInvalidPacketEvent(object[] args)
        {
            if (!FireEvent(InvalidPacket, args))
            {
                ...
                eventLog.WriteEntry("InvalidPacketEvent - " + ea.Reason,
        ...
        internal static void RaiseInvalidPacketInFrameEvent(object[] args)
        {
            if (!FireEvent(InvalidPacketInFrame, args))
            {
                ...
                eventLog.WriteEntry(string.Format("InvalidPacketInFrameEvent - RtpStream:{0}, Reason:{1}",
        ...
        internal static void RaiseNetworkTimeoutEvent(object[] args)
        {
            if (!FireEvent(NetworkTimeout, args))
            {
                ...
                eventLog.WriteEntry("NetworkTimeoutEvent - " + ea.Message,
                    EventLogEntryType.Warning, (int)RtpEL.ID.NetworkTimeout);
            }
        }
        ...
        internal static void RaisePacketOutOfSequenceEvent(object[] args)
        {
            if (!FireEvent(PacketOutOfSequence, args))
            {
                ...
                eventLog.WriteEntry(string.Format("PacketOutOfSequenceEvent - " +
                    "RtpStream:{0}, LostPackets:{1}, Message:{2}",
                    ea.RtpStream, ea.LostPackets, ea.Message),
                    EventLogEntryType.Warning, (int)RtpEL.ID.PacketOutOfSequence);
            }
        }
        ...
        internal static void RaiseRtpParticipantAddedEvent(object[] args)
        {
            ...
            eventLog.WriteEntry("RtpParticipantAddedEvent - " + ea.RtpParticipant.ToString(),
                EventLogEntryType.Information, (int)RtpEL.ID.RtpParticipantAdded);

            if (!FireEvent(RtpParticipantAdded, args))
            {
                eventLog.WriteEntry("Event not hooked - RtpParticipantAddedEvent - " + ea.RtpParticipant.ToString(),
        ...
        internal static void RaiseReceiverReportEvent(object[] args)
        {
            ...
            //            if(!FireEvent(ReceiverReport, args))
            //            {
            //                ReceiverReportEventArgs ea = (ReceiverReportEventArgs)args[1];
            //
            //                eventLog.WriteEntry(string.Format("RtpStreamReceiverReportEvent - Source:{0}, Sender:{1}, Report:{2}", 
            //                    ea.SourceCName, ea.SenderCName, ea.ReceiverReport),
            //                    EventLogEntryType.Information, (int)RtpEL.ID.RtpStreamReceiverReport);
            //            }
        }


        #endregion RtpStreamReceiverReport
        ...
        internal static void RaiseRtpStreamAddedEvent(object[] args)
        {
            ...
            eventLog.WriteEntry("RtpStreamAddedEvent - " + ea.RtpStream.ToString(),
                EventLogEntryType.Information, (int)RtpEL.ID.RtpStreamAdded);

            if (!FireEvent(RtpStreamAdded, args))
            {
                eventLog.WriteEntry("Event not hooked - RtpStreamAddedEvent" + ea.RtpStream.ToString(),
                    EventLogEntryType.Warning, (int)RtpEL.ID.EventNotHooked);
            }
        }
        ...
        internal static void RaiseRtpStreamRemovedEvent(object[] args)
        {
            ...
            eventLog.WriteEntry("RtpStreamRemovedEvent - " + ea.RtpStream.ToString(),
                EventLogEntryType.Information, (int)RtpEL.ID.RtpStreamRemoved);

            if (!FireEvent(RtpStreamRemoved, args))
            {
                eventLog.WriteEntry("Event not hooked - RtpStreamRemovedEvent - " + ea.RtpStream.ToString(),
                    EventLogEntryType.Warning, (int)RtpEL.ID.EventNotHooked);
            }
        }
        ...
        internal static void RaiseRtpStreamTimeoutEvent(object[] args)
        {
            ...
            eventLog.WriteEntry("RtpStreamTimeoutEvent - " + ea.RtpStream.ToString(),
                EventLogEntryType.Warning, (int)RtpEL.ID.RtpStreamTimeout);

            if (!FireEvent(RtpStreamTimeout, args))
            {
                eventLog.WriteEntry("Event not hooked - RtpStreamTimeoutEvent - " + ea.RtpStream.ToString(),
                    EventLogEntryType.Warning, (int)RtpEL.ID.EventNotHooked);
            }
        }
        ...
        internal static bool FireEvent(Delegate del, object[] args)
        {
            ...
            if (del != null)
            {
                ...
                foreach (Delegate sink in sinks)
                {
                    ...
                    {
                        /// $CompactConferenceXP:
                        /// There is not Delegate.DynamicInvoke
                        /// Replaced
                        /// > sink.DynamicInvoke(args);
                        /// >> CCXP Correction: del.GetType().GetMethod("Invoke").Invoke(del, args);
                        sink.GetType().GetMethod("Invoke").Invoke(sink, args);                        
                    }
                    catch (Exception e)
                    {
                        eventLog.WriteEntry("Error calling an event delegate - " + e.ToString(),
